...
impl NormalizedQuote {
    ...
    /// Converts this normalized quote to a Quote using normalized values.
    ///
    /// Creates a Quote where:
    /// - price = all_in_price (includes fees for fair comparison)
    /// - quantity = normalized_quantity (contract-adjusted)
    ///
    /// This enables transparent normalization in the aggregation pipeline
    /// while preserving compatibility with existing ranking strategies.
    ///
    /// # Note
    ///
    /// The returned Quote will have a new ID and timestamp generated by the
    /// builder. The original quote ID and timestamps are preserved in this
    /// NormalizedQuote instance.
    #[must_use]
    #[inline]
    pub fn to_quote(&self) -> crate::domain::entities::quote::Quote {
        ...
        .build()
        .with_last_look_required(self.requires_last_look)
    }
}
